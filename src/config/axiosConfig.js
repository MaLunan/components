...
const SSO_LOGIN_TEST = 'http://test.ssa.jd.com/sso/login?ReturnUrl=http%3a%2f%2ftest.compliance.jr.jd.com',
    SSO_LOGIN_PROD = 'https://ssa.jd.com/sso/login?ReturnUrl=http%3a%2f%2fcollect.ticket.jr.jd.com';
  ...
  if (error && res) {
    switch (res.status) {
      ...
      case 401:
        ...
        console.log("收到401响应码，响应头location=" + location);
        if (location !== undefined && location.indexOf('test') !== -1) {
          window.location.href = SSO_LOGIN_TEST;
          // window.location.href = SSO_LOGIN_TEST
        }else{
          window.location.href = SSO_LOGIN_PROD;
        }
        break
      case 403:
        error.message = '拒绝访问'
